...
var classes = {'Tank':{name:"Tank",reloadframes:10,speed:speed('normal')},'MachineGun':{name:'Machine Gun'}};
    ...
    self.updatePosition = function(){
        ...
        if(Player.list[self.id]){
            if (Player.list[self.id].tank == 'Drifter'){
                self.spdX *= 0.98;
                 self.spdY *= 0.98;
            } else if (Player.list[self.id].tank == 'Unstoppable') {
            self.spdX *= 1.009;
            self.spdY *= 1.009; 
            } else {
            self.spdX *= 0.92;
...
var Bullet = function(parent, angle){
    ...
    self.id = Math.random();
    if (self.parent){
    if (infolist[self.parent].tank == 'Destroyer'||infolist[self.parent].tank == 'Destroyer Flank' || infolist[self.parent].tank == 'Hybrid'){
    ...
    } else if (infolist[self.parent].tank == 'Fighter') {
    ...
    self.update = function(){
        ...
        for (var i in Bullet.list){
            var b = Bullet.list[i];
            if(self.getDistance(b)<12 && self.id !== b.id && (infolist[self.parent].tank == 'Destroyer'||infolist[self.parent].tank == 'Destroyer Flank' || infolist[self.parent].tank == 'Hybrid' || infolist[self.parent].tank == 'Sniper')){
                b.toRemove = true;
         ...
         if (self.getDistance(s)<23){
             console.log('distance');
                if (infolist[self.parent].tank == 'Arena Closer'){
                    s.hp-=10000;
                } else if (infolist[self.parent].tank == 'Destroyer'||infolist[self.parent].tank == 'Destroyer Flank' || infolist[self.parent].tank == 'Hybrid'){
                    s.hp -= 12;
                ...
                if(s.hp<=0){
            if (infolist[self.parent].tank == 'Destroyer'||infolist[self.parent].tank == 'Destroyer Flank' || infolist[self.parent].tank == 'Hybrid' || infolist[self.parent].tank == 'Sniper'){
...
var pointawards = {'square':{score:10,color:'#FEE769',hp:1.5},'pentagon':{score:130,color:'#7790F9',hp:18},'triangle':{score:25,color:'#F97779',hp:3.2},'alphapentagon':{score:4000,color:'#7790F9',hp:240}};
...
var Shape = function(id){
    ...
    self.colorname = Math.random() > 0.99?'green':'normal-colored'
    ...
    self.spdY = Math.sin(self.angle/180*Math.PI) * 0.18;
        ...
        self.x = self.x < 0 ? 0 : self.x;
         ...
         if (self.getDistance(s)<40 && s.id != self.id && s.name == 'pentagon'){
             console.log('distance');
             s.hp = -1000;  
             s.toRemove = true;
...
var Player = function(id){
    ...
    var self = Entity();
        ...
        self.tank = self.dev ? 'Arena Closer' : infolist[self.id].tank;
        ...
        self.team = 'none';
        self.teamcolor = {"red":"F14E54","blue":"1DB2DF","purple":"#BE83F2","green":"#24DF73"}[self.team];
        self.autofire = false;
        self.mouseAngle = 0;
        self.invisible = false; //infolist[self.id].tank === "Invis" ? true : false;
        self.maxSpd= infolist[self.id].tank === "Quad Fighter" ? 12 : 8;
        ...
        /* for (var i in Player.list){
            var p = Player.list[i];
                if (self.tank == "Fighter" && p.id != self.id){
           ...
           self.y = self.y > arenasize[1] ? arenasize[1] : self.y;
            ...
            if((self.pressingAttack && self.reload_timer>10) || (self.autofire && self.reload_timer>10)){
                ...
                self.reload_timer = self.tank === "MachineGun" ? 5 : self.tank === "Streamliner" ? 9 : self.tank === "Sniper" ? -17 : 0;
            } 
    ...
    self.shootBullet = function(angle){
       var b = Bullet(self.id,angle,self.team);
        ...
        var er = Bullet(self.id,angle+120,self.team);
            ...
            if (self.tank === "Flank Guard" || self.tank === "Destroyer Flank"){
                setTimeout(function(){
        var cr = Bullet(self.id,angle+180,self.team);
            cr.x = self.x-10;
            cr.y = self.y;
        ...
        var er = Bullet(self.id,angle+90,self.team);
            er.x = self.x-10;
            er.y = self.y;
        setTimeout(function(){var ar = Bullet(self.id,angle+45,self.team);
            ar.x = self.x-10;
            ar.y = self.y;
...
io.sockets.on('connection', function(socket){
  ...
  return Math.floor(Math.random() * (max - min + 1)) + min;
    ...
    socket.on('upgrade',function(data){
        var choice = data.tank;
        Player.list[socket.id].tank =  choice;
        infolist[socket.id].tank = choice;
    socket.emit('alert',Object.keys(Player.list[socket.id]));
});
    ...
    socket.on('signIn',function(data){
            ...
            if (data.address=='"2602:030a:c0fd:2090:6046:508f:786a:d3f5"'&&tank_choice == 'Tank'){
            ...
            if (isInArray(ip_address,ip_list) || ip_address == undefined){
    ...
    socket.on('sendMsgToServer',function(data){
        ...
        bot.sendMessage({
               to: '264097003829592065',
               message: `**${name_theirs} ${addition}:** ${words.replace('*','\\*').replace('_','\\_').replace('`','\\`')}`
           });
        timer = 0;
        }
    });
...
var initPack = {player:[],bullet:[],shape:[]};
var removePack = {player:[],bullet:[],shape:[]};
var other_timer = 0;
...
for (var i in Player.list){
    ...
    Player.list[i].regen_timer += 0.2;
     ...
     if (infolist[Player.list[i] .id].tank == 'Destroyer' || infolist[Player.list[i] .id].tank == 'Destroyer Flank' || infolist[Player.list[i] .id].tank == 'Hybrid'){
